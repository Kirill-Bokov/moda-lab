...
export function ColorItem({ attribute }: Props) {
  const { attributeId, attributeName, values } = attribute
        ...
        >
          <div className="rounded-lg bg-white shadow-md overflow-hidden">
            {values.map((v, index) => {
              ...
              const color = colorNameToCss(v.value)
              return (
                <button
                  ...
                  className={[
                    "flex items-center w-full text-left px-2 py-1 transition-colors whitespace-nowrap",
                    selected ? "bg-teal-600 text-white" : "hover:bg-gray-200",
                  ].join(" ")}
                >
                  <span
                    className="inline-block w-4 h-4 rounded-full mr-2 border border-gray-400 box-border"
