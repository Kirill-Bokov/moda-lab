...
export const generateProducts = (
  ...
  for (let i = 0; i < total; i++) {
    ...
    // Генерация рейтинга 1.0 - 7.0
    const rating =
      Math.round((1 + Math.random() * 6) * 10) / 10
    ...
    items.push({
      ...
      variant_ratings_number: ratingsNumber,
    })
  }
...
export const productByCategoryMock: Record<
  number,
  ProductsByCategoryResponse
> = [401, 402, 403, 404, 405].reduce(
  (acc, categoryId) => {
